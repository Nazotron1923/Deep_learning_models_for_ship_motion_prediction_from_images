...
def train(inputs, targets, CNN_p, LSTM_encoder, LSTM_decoder, CNN_optimizer,
        LSTM_encoder_optimizer, LSTM_decoder_optimizer, criterion,
...
def eval(inputs, targets, CNN_p, LSTM_encoder,
         LSTM_decoder, criterion, time_gap=5, frame_interval= 12, sec_to_pred = 2):
...
def main(train_folder, num_epochs=100, batchsize=32, learning_rate=0.001, seed=42,
    ...
    LSTM_encoder_p = LSTM_encoder(input_size=sec_to_pred*int(24/frame_interval)*1024, hidden_size=1024, num_layers=1)
    LSTM_decoder_total = LSTM_decoder(hidden_size=1024, output_size = 2*im_to_predict)
    CNN_p = AutoEncoder()
    ...
    file_CNN_model = "CNN_LSTM_simple_without_PR_CNN_part_{}s_{}_s_to_pred_lr_{}_tmp.pth".format(time_gap, sec_to_pred, learning_rate)
    file_encoder_model = "CNN_LSTM_simple_without_PR_Encoder_part_{}s_{}_s_to_pred_lr_{}_tmp.pth".format(time_gap, sec_to_pred, learning_rate)
    file_decoder_model = "CNN_LSTM_simple_without_PR_Decoder_part_{}s_{}_s_to_pred_lr_{}_tmp.pth".format(time_gap, sec_to_pred, learning_rate)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for i, (inputs, targets) in enumerate(train_loader):
            ...
            loss = train(inputs, targets, CNN_p, LSTM_encoder_p,
                        LSTM_decoder_total, CNN_optimizer, LSTM_encoder_optimizer,
        ...
        for i, (inputs, targets) in enumerate(val_loader):
            ...
            # Convert to pytorch variables
            inputs, targets = Variable(inputs), Variable(targets)
            loss = eval(inputs, targets, CNN_p, LSTM_encoder_p,
                        LSTM_decoder_total, criteration, time_gap = time_gap, sec_to_pred = sec_to_pred)
            val_loss += loss
    ...
    plt.savefig(RES_DIR+'CNN_LSTM_simple_without_PR_model_'+'_predict_'+str(time_gap)
    ...
    origin_names = [RES_DIR+'cnn_lstm_simple_without_PR_origin_label_use_'+str(sec_to_pred)+'_to_predict_'+str(i+1)+':'+str(im_to_predict)+'_lr_'+str(learning_rate)+'_.json' for i in range(im_to_predict)]
    preds = [{} for i in range(im_to_predict)]
    pred_names = [RES_DIR+'cnn_lstm_simple_without_PR_pred_label_use_'+str(sec_to_pred)+'_to_predict_'+str(i+1)+':'+str(im_to_predict)+'_lr_'+str(learning_rate)+'_.json' for i in range(im_to_predict)]
    ...
    with open("./Pre/result_CNN_LSTM_simple_without_PR.txt", "a") as f:
        ...
        f.write(str(CNN_p))
        f.write("\n")
        f.write(str(LSTM_encoder_p))
        f.write("\n")
        f.write(str(LSTM_decoder_total))
        f.write("\n")
        f.write(str(CNN_optimizer))
        f.write("\n")
        f.write(str(LSTM_encoder_optimizer))
        f.write("\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--time_gap', help='Time in seconds to predict ', default=4, type=int)
