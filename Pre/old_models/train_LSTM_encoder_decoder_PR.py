...
def train(inputs, targets, CNN_p, LSTM_encoder, LSTM_decoder, CNN_optimizer,
    ...
    for im in range(use_n_im-1, target_length+use_n_im):
        ...
        lstm_input_features = th.cat(PR, 1).view(inputs.size(0), 1, -1)
...
def main(train_folder, num_epochs=100, batchsize=32, learning_rate=0.001, seed=42,
    ...
    LSTM_encoder_p = LSTM_encoder(input_size=sec_to_pred*int(24/frame_interval)*2, hidden_size=300, num_layers=1)
    LSTM_decoder_total = LSTM_decoder(hidden_size=300, output_size = 2*im_to_predict)
    CNN_p = AutoEncoder()
    ...
    file_CNN_model = "CNN_LSTM_simple_without_im_CNN_part_{}s_{}_s_to_pred_lr_{}_tmp.pth".format(time_gap, sec_to_pred, learning_rate)
    file_encoder_model = "CNN_LSTM_simple_without_im_Encoder_part_{}s_{}_s_to_pred_lr_{}_tmp.pth".format(time_gap, sec_to_pred, learning_rate)
    file_decoder_model = "CNN_LSTM_simple_without_im_Decoder_part_{}s_{}_s_to_pred_lr_{}_tmp.pth".format(time_gap, sec_to_pred, learning_rate)
    ...
    plt.savefig(RES_DIR+'CNN_LSTM_simple_without_im_model_'+'_predict_'+str(time_gap)
    ...
    origin_names = [RES_DIR+'cnn_lstm_simple_without_im_origin_label_use_'+str(sec_to_pred)+'_to_predict_'+str(i+1)+':'+str(im_to_predict)+'_lr_'+str(learning_rate)+'_.json' for i in range(im_to_predict)]
    preds = [{} for i in range(im_to_predict)]
    pred_names = [RES_DIR+'cnn_lstm_simple_without_im_pred_label_use_'+str(sec_to_pred)+'_to_predict_'+str(i+1)+':'+str(im_to_predict)+'_lr_'+str(learning_rate)+'_.json' for i in range(im_to_predict)]
    ...
    with open("./Pre/result_CNN_LSTM_without_im.txt", "a") as f:
