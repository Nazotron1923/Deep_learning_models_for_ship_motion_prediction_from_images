...
def train(inputs, targets, CNN_p, LSTM_encoder_Images, LSTM_encoder_PR,
          LSTM_decoder_p, CNN_optimizer, LSTM_encoder_Images_optimizer,
          LSTM_encoder_PR_optimizer, LSTM_decoder_optimizer, criterion,
          time_gap=5, frame_interval= 12,  sec_to_pred = 2):

    encoder_hidden_images = (LSTM_encoder_Images.initHidden(inputs.size(0)).cuda(),
                            LSTM_encoder_Images.initHidden(inputs.size(0)).cuda())
    encoder_hidden_PR = (LSTM_encoder_PR.initHidden(targets.size(0)).cuda(),
                        LSTM_encoder_PR.initHidden(targets.size(0)).cuda())
    decoder_hidden = (LSTM_decoder_p.initHidden(targets.size(0)).cuda(),
                    LSTM_decoder_p.initHidden(targets.size(0)).cuda())

    CNN_optimizer.zero_grad()
    LSTM_encoder_Images_optimizer.zero_grad()
    LSTM_encoder_PR_optimizer.zero_grad()
    ...
    for im in range(use_n_im-1, target_length+use_n_im):
        ...
        lstm_input_features = th.cat(tuple(features), 1).view(inputs.size(0), 1, -1)
        lstm_input_PR = th.cat(PR, 1).view(inputs.size(0), 1, -1)

        encoder_output_images, encoder_hidden_images = LSTM_encoder_Images(lstm_input_features,  encoder_hidden_images )
        encoder_output_PR, encoder_hidden_PR = LSTM_encoder_PR(lstm_input_PR,  encoder_hidden_PR )

        lstm_input_decoder = th.cat((encoder_output_images, encoder_output_PR), 2).view(inputs.size(0), 1, -1)

        decoder_output, decoder_hidden = LSTM_decoder_p(lstm_input_decoder, decoder_hidden)
    ...
    CNN_optimizer.step()
    LSTM_encoder_Images_optimizer.step()
    LSTM_encoder_PR_optimizer.step()
    LSTM_decoder_optimizer.step()

    return loss.item() / target_length


def eval(inputs, targets, CNN_p, LSTM_encoder_Images, LSTM_encoder_PR,
         LSTM_decoder_p, criterion, time_gap=5, frame_interval= 12, sec_to_pred = 2):

    encoder_hidden_images = (LSTM_encoder_Images.initHidden(inputs.size(0)).cuda(), LSTM_encoder_Images.initHidden(inputs.size(0)).cuda())
    encoder_hidden_PR = (LSTM_encoder_PR.initHidden(targets.size(0)).cuda(), LSTM_encoder_PR.initHidden(targets.size(0)).cuda())
    decoder_hidden = (LSTM_decoder_p.initHidden(targets.size(0)).cuda(), LSTM_decoder_p.initHidden(targets.size(0)).cuda())
    ...
    with th.no_grad():
        ...
        for im in range(use_n_im-1, target_length+use_n_im):
            ...
            lstm_input_features = th.cat(features, 1).view(inputs.size(0), 1, -1)
            lstm_input_PR = th.cat(PR, 1).view(inputs.size(0), 1, -1)
...
def test(i, origins, preds, batchsize, inputs, targets,
        CNN_p, LSTM_encoder_Images, LSTM_encoder_PR, LSTM_decoder_p,
...
def main(train_folder, num_epochs=100, batchsize=32, learning_rate=0.001, seed=42,
        cuda=True, num_output=2, random_trans=0.0, evaluate_print=1,
    ...
    LSTM_encoder_Images = LSTM_encoder(input_size=sec_to_pred*int(24/frame_interval)*1024, hidden_size=600, num_layers=1)
    LSTM_encoder_PR = LSTM_encoder(input_size=sec_to_pred*int(24/frame_interval)*2, hidden_size=300, num_layers=1)
    LSTM_decoder_total = LSTM_decoder(hidden_size=900, output_size = 2*im_to_predict)
    CNN_p = AutoEncoder()
    ...
    if cuda:
        LSTM_encoder_Images.cuda()
        LSTM_encoder_PR.cuda()
    ...
    LSTM_encoder_Images_optimizer = th.optim.Adam(LSTM_encoder_Images.parameters(), lr=learning_rate, weight_decay=weight_decay)
    LSTM_encoder_PR_optimizer = th.optim.Adam(LSTM_encoder_PR.parameters(), lr=learning_rate, weight_decay=weight_decay)
    ...
    file_CNN_model = "CNN_LSTM_CNN_part_{}s_{}im_to_pred_tmp.pth".format(time_gap, num_images)
    file_encoder_image_model = "CNN_LSTM_Encoder_Image_part_{}s_{}im_to_pred_tmp.pth".format(time_gap, num_images)
    file_encoder_PR_model = "CNN_LSTM_Encoder_PR_part_{}s_{}im_to_pred_tmp.pth".format(time_gap, num_images)
    file_decoder_model = "CNN_LSTM_Decoder_part_{}s_{}im_to_pred_tmp.pth".format(time_gap, num_images)

    best_model_CNN_part = RES_DIR + file_CNN_model
    best_model_encoder_im_part = RES_DIR + file_encoder_image_model
    best_model_encoder_PR_part = RES_DIR + file_encoder_PR_model
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        LSTM_encoder_Images.train()
        LSTM_encoder_PR.train()
        ...
        for i, (inputs, targets) in enumerate(train_loader):
            ...
            loss = train(inputs, targets, CNN_p, LSTM_encoder_Images,
                        LSTM_encoder_PR, LSTM_decoder_total, CNN_optimizer,
                        LSTM_encoder_Images_optimizer, LSTM_encoder_PR_optimizer,
                        LSTM_decoder_optimizer, criteration, time_gap = time_gap, sec_to_pred = sec_to_pred)
            train_loss += loss

        train_l = train_loss / (n_train*batchsize)
        LSTM_encoder_Images.eval()
        LSTM_encoder_PR.eval()
        ...
        for i, (inputs, targets) in enumerate(val_loader):
            ...
            # Convert to pytorch variables
            inputs, targets = Variable(inputs), Variable(targets)
            loss = eval(inputs, targets, CNN_p, LSTM_encoder_Images,
                        LSTM_encoder_PR, LSTM_decoder_total, criteration, time_gap = time_gap, sec_to_pred = sec_to_pred)
            val_loss += loss
        ...
        if val_l < best_error:
            ...
            th.save(LSTM_encoder_Images.state_dict(), best_model_encoder_im_part)
            th.save(LSTM_encoder_PR.state_dict(), best_model_encoder_PR_part)
    ...
    plt.savefig(RES_DIR+'CNN_LSTM_model_'+'_predict_'+str(time_gap)
                            +'s_using_'+str(sec_to_pred)+'_lr_'+str(learning_rate)+'s_loss'+'.png')
    # LOQD the best model
    CNN_p.load_state_dict(th.load(best_model_CNN_part))
    LSTM_encoder_Images.load_state_dict(th.load(best_model_encoder_im_part))
    LSTM_encoder_PR.load_state_dict(th.load(best_model_encoder_PR_part))
    ...
    origin_names = [RES_DIR+'origin_label_use_'+str(sec_to_pred)+'_to_predict_'+str(i+1)+':'+str(im_to_predict)+'_lr_'+str(learning_rate)+'_.json' for i in range(im_to_predict)]
    preds = [{} for i in range(im_to_predict)]
    pred_names = [RES_DIR+'pred_label_use_'+str(sec_to_pred)+'_to_predict_'+str(i+1)+':'+str(im_to_predict)+ '_lr_'+str(learning_rate)+'_.json' for i in range(im_to_predict)]
    ...
    for i , (inputs, targets) in enumerate(test_loader):
        ...
        loss, origins, preds  = test(i, origins, preds, batchsize, inputs,
                                    targets, CNN_p, LSTM_encoder_Images,
                                    LSTM_encoder_PR, LSTM_decoder_total, criteration,
    ...
    with open("./Pre/result_CNN_LSTM.txt", "a") as f:
        ...
        f.write(str(CNN_p))
        f.write("\n")
        f.write(str(LSTM_encoder_Images))
        f.write("\n")
        f.write(str(LSTM_encoder_PR))
        f.write("\n")
        f.write(str(LSTM_decoder_total))
        f.write("\n")
        f.write(str(CNN_optimizer))
        f.write("\n")
        f.write(str(LSTM_encoder_Images_optimizer))
        f.write("\n")
        f.write(str(LSTM_encoder_PR_optimizer))
        f.write("\n")
