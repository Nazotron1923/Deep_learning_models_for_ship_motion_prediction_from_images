...
parser.add_argument('-ni', '--num_images', help='Number of images in the series', default=60, type=int)
parser.add_argument('-o', '--origin_file', help='Original file', default="Pre/results/origin_test.json", type=str)
parser.add_argument('-p', '--prediction_file', help='Predictin file', default="Pre/results/pred_test.json", type=str)
...
str_t = str(RES_DIR+args.model+'_KAMINSKYI_plt_test_'+str(args.time_gap)+'s_'+str(args.num_images)+'im_roll_final'+'.png')
plt.savefig(str_t)
plt.show()
...
plt.ylabel("Pitch")
plt.legend(loc='upper right')
plt.savefig(RES_DIR+args.model+'_KAMINSKYI_plt_test_'+str(args.time_gap)+'s_'+str(args.num_images)+'im_pitch_final'+'.png')
